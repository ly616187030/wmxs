...
class CanmingController extends AdminController{
    ...
    public function index(){
		...
		$sname = I('storename');
		if(empty($storeid) && (is_administrator() || is_founder())){
    ...
    public function add(){
	        if(IS_POST){
	            ...
	            if($m->create()){
                    ...
                    $add_id = $m->add();
	                if($add_id > 0){
                        action_log('add_action','Canming',$add_id,is_login());
	        ...
	        if(IS_POST){
				...
				if($m->create()){
	                ...
	                if($arr !== false){
	                    ...
	                    $this->success('更新数据成功','javascript:history.back(-1);');
	                }else{
    ...
    public function del(){
        ...
        if($list>0){
            action_log('delete_action','Canming',is_array($id)?implode(',', $id):$id,is_login());
            $this->success('删除成功');
