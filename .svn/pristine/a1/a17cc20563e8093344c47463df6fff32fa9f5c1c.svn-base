...
class AccessController extends AdminController {
    ...
    public function index() {
        ...
        if(!is_administrator()){
            ...
            $allgid = D('Admin/Group')->getGroups(UID);
            /*$ids = listGroups($allgid,$gid);
            $ids = array_merge($ids,array($gid));*/
            $map['group'] = array('in',$allgid);
        }
    ...
    public function add(){
        if (IS_POST) {
            ...
            if(!is_administrator()){
                $gid=D('Admin/Group')->getGroups(UID);
                $gid=implode(',',$gid);
                $map['group'] = array('in',$gid);
            }else{
                $map=null;
            }
            ...
            $builder->setMetaTitle('新增配置')  //设置页面标题
                    ...
                    ->addFormItem('group', 'select', '用户组', '不同用户组对应相应的权限', select_list_as_tree('Group',$map))
                    ->display();
        }
    }
