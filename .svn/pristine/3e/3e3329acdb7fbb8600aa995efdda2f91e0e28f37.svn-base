...
class LunboController extends AdminController{

    //初始化

    public function index(){

        $weixin = M('lunbo_weixin'); // 实例化User对象
...
//		$list = $jokes->alias('s')->join('LEFT JOIN wm_city AS c ON s.city_id=c.city_id')->field('s.*,c.city_name')->order('sort_order')->select();
    ...
    public function toogleHide($id,$value = 1){
        $this->editRow('lunbo_weixin', array('flag'=>$value), array('lb_id'=>$id));
    }
    ...
    public function add(){
        ...
        if(IS_POST){
            $rules = array(
                array('lb_pic','require','轮播图片不能为空！'),
            );
            $weixin = M("lunbo_weixin");//动态验证
    ...
    public function del(){
        ...
        $id = array_unique((array)I('lb_id',0));
        $map = array('lb_id' => array('in', $id) );
        if(M('lunbo_weixin')->where($map)->delete()){
            $this->success('删除成功');
        }else{
            $this->error('删除失败！');
        }
    ...
    public function edit(){
        ...
        if(IS_POST){
            ...
            $id = I('get.lb_id');
            $data = $weixin->where('lb_id='.$id)->find();/* 获取数据 */
