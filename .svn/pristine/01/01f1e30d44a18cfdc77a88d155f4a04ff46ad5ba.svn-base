...
class TypeController extends AdminController {
    ...
    public function index() {
        ...
        $attr['href']  = U('User/Admin/Attribute/index', array('user_type' => __data_id__));
    ...
    public function add() {
        if (IS_POST) {
            ...
            $builder->setMetaTitle('新增用户类型') //设置页面标题
                    ...
                    ->addFormItem('name', 'text', '名称', '用户类型名称如：manage,member')
                    ->addFormItem('title', 'text', '标题', '用户类型标题')
                    ->addFormItem('status','radio','是否开启','',array('1'=>'开启','0'=>'关闭'))
                    ->display();
        }
    }
