...
class UserModel extends Model {
     ...
     * @author jry <598821125@qq.com>
     */
    protected $tableName = 'member_user';
    ...
    protected $_validate = array(
        ...
        // 验证用户名
        array('username', 'require', '请填写登录账号', self::MUST_VALIDATE, 'regex', self::MODEL_INSERT),
        ...
        // 验证邮箱
        array('email', 'email', '邮箱格式不正确', self::VALUE_VALIDATE, 'regex', self::MODEL_INSERT),
        array('email', '1,32', '邮箱长度为1-32个字符', self::VALUE_VALIDATE, 'length', self::MODEL_INSERT),
        array('email', '', '邮箱被占用', self::VALUE_VALIDATE, 'unique', self::MODEL_INSERT),
    ...
    public function auto_login($user) {
        ...
        /*if(empty(cookie('remember_login'))){
            session('user_auth',$auth);
            session('user_auth_sign', $this->data_auth_sign($auth));
        }else{
            cookie('user_auth', serialize($auth),86400000);
            cookie('user_auth_sign', $this->data_auth_sign($auth),86400000);
        }*/
        return true;
    }
