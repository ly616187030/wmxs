$(function(){
    ...
    // 打开新Tab
    $('body').delegate('#sidebar .open-tab,.dropdown-menu .open-tab', 'click', function() {
    ...
    $('body').delegate('.nav-close .close', 'click', function() {
        ...
        var xx = $('.nav-close .close').length;
        if(xx==0){
            $('.ct-tab a:first').tab('show').click();
        }
    });
    ...
    $('body').delegate('.close-all', 'click', function() {
        $('.new-add').remove();
        $('.navside-nav > li').removeClass('active');
        $('.ct-tab a:first').tab('show').click();
