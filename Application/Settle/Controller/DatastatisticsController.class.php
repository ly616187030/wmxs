...
class DatastatisticsController extends AdminController{
    ...
    public function dailyData(){
        ...
        $uid=is_login();
        $user = M('admin_user');
        $admin=$user->where('id='.$uid)->field('user_type,founder_id,store_list')->find();
        if($admin['user_type']=='company'){
            $condition['founder_id']=$admin['founder_id'];
            $data=M('store')->where($condition)->getField('store_id',true);

            if($data){
                $where['a.store_id']=array('in',$data);
                $where20['founder_id']=$admin['founder_id'];
                $list_s = $s->where($where20)->select();//查询所有的商家
                $list_dp = $dp->where($where20)->select();//查询所有的配送员
            }
        }
        if($admin['user_type']=='company_member'){
            if($admin['store_list']){
                $where['a.store_id']=array('in',$admin['store_list']);
                $where21['founder_id']=FID;
                $list_dp = $dp->where($where21)->select();//查询所有的配送员
                $list_s = $s->where($where21)->select();//查询所有的商家
            }else{
                $condition['uid']=$uid;
                $data=M('store')->where($condition)->getField('store_id',true);
                if($data){
                    $where['a.store_id']=array('in',$data);
                    $list_dp = $dp->where('uid='.$uid)->select();//查询所有的配送员
                    $list_s = $s->where('uid='.$uid)->select();//查询所有的商家
                }
            }
        }
...
//            $person_id = $m->distinct(true)->where($where)->field('person_id')->select();
            ...
            $arr[$i]['person'] = round($sum/$count,2);//订单均价
            $arr[$i]['order'] = round($count,2);//订单数量
