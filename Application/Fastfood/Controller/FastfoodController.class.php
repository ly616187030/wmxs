...
class FastfoodController extends AdminController{
    ...
    public function setOrder(){
        ...
        $order_count_where['from_operation'] = 4;
        ...
        if($n->create()){
            ...
            if($order > 0){
                ...
                for($i=0;$i<count($cm_id);$i++){
                    ...
                    $od->price = $newdj[$i];
                    $od->quantity = $num[$i];
                    $od->total_money = $newzj[$i];
                    $od->xd_time = $list_od['xd_time'];

                    $this->updateGoodsSale($gs,$store_id,$cm_id[$i],$num[$i]);//菜品数量写入统计表
                ...
                if(!empty($seat_id)){
                    $sn->where('id = '.$seat_id)->setField('status',1);//订单成功以后更新桌台状态
                }
                if($list_d > 0){
                    $this->ajaxReturn(1);
                }
    ...
    public function setUser(){
        ...
        $peisong_price = $s->where('store_id = '.$store_id)->getField('peisong_price');
        $store_sn = $s->where('store_id = '.$store_id)->getField('store_sn');
        ...
        if($m->create() && $n->create()){
            ...
            if($list > 0){
                ...
                $n->status = 3;
                $n->pay_type = 0;
                $n->send_time = strtotime(I('post.send_time'));
                $n->sh_mode = 2;
                $n->from_operation = 4;
                $n->address_id = $list;
                $n->canju_total = round($canju_total,2);
                $n->ps_cost = $peisong_price;
                $n->order_count = $count;
                $n->store_sn = $store_sn;
                $order = $n->add();
                if($order > 0){
